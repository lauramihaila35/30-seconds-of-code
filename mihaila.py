...
with SB(uc=True, test=True) as sb:
    ...
    if True:
        ...
        if sb.is_element_visible('#injected-channel-player'):
            vvvvv = sb.get_new_driver(undetectable=True)
            vvvvv.uc_open_with_reconnect(url, 5)
            vvvvv.uc_gui_click_captcha()
            vvvvv.uc_gui_handle_captcha()
            sb.sleep(10)
            if vvvvv.is_element_present('button:contains("Accept")'):
                vvvvv.uc_click('button:contains("Accept")', reconnect_time=4)
